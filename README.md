...
![image](https://github.com/user-attachments/assets/0c5499e0-4344-4dc0-a2f8-0c5c270a15d0)
- **오늘의 키워드**  
  - 최근 24시간(Asia/Seoul 기준) 생성된 클러스터들의 대표 키워드와 키워드 간의 관계를 제공합니다.
...
- **주간 트렌드**  
  - 설명: 지난 7일간(오늘 미포함) `cluster_keyword`별 총 등장 횟수를 집계하여 상위 키워드와 각 키워드의 일별 등장 추이를 시계열 그래프로 제공합니다

![image](https://github.com/user-attachments/assets/5d954795-f5e2-482c-bc0d-5723d993e54c)
- **트렌드 키워드 목록**  
  - 설명: 일주일(오늘 미포함) 동안 집계된 모든 키워드를 총 가나다 기준으로 내림차순 정렬한 리스트를 제공합니다
- **트렌드 키워드 검색**  
  - 설명: 관심 있는 키워드를 클릭하면, 해당 키워드의 최근 7일간 일별 등장 횟수와 연관 클러스터·대표 키워드를 함께 확인할 수 있습니다 
...
### 4. 스크랩 & 노트  
- **기사 스크랩**  
  - 뉴스 목록 또는 클러스터 상세 화면에서 스크랩 버튼을 클릭해 관심 기사를 즐겨찾기에 추가합니다
...
- **스크랩 취소**  
  - 스크랩 버튼을 다시 클릭하면 해당 기사가 스크랩 목록에서 삭제됩니다
...
### 5. 지식맵  
- **자동 생성**  
  - 사용자별 스크랩 데이터를 기반으로 일정 주기마다 상위 키워드를 추출하고, 키워드 간 유사도 기반 간선을 생성하여 지식맵을 구축합니다 
- **지식맵 조회**  
