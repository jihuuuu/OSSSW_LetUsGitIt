...
export default function ClusterDetailPage() {
  ...
  useEffect(() => {
    axios.get<ClusterDetail>(`http://3.39.180.27:8000/clusters/today/${clusterId}/articles`).then((res) => {
      setCluster(res.data);
    });

  const fetchData = async () => {
    try {
      const res = await axios.get<{ articles: any[] }>("http://3.39.180.27:8000/users/scraps", {
        headers: {
  ...
  const handleScrap = async (articleId: number) => {
    ...
    try {
      const url = `http://3.39.180.27:8000/users/articles/${articleId}/${isScrapped ? "unscrap" : "scrap"}`;
