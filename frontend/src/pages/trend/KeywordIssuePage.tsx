...
export default function KeywordIssuePage() {
  ...
  useEffect(() => {
    fetch("${import.meta.env.VITE_API_URL}/trends/suggested_keywords")
    .then((res) => {
  ...
  const handleSearch = async (kw: string) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/trends/search?keyword=${encodeURIComponent(kw)}`);
